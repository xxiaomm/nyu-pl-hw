...
case class Leaf[T] (x: T) extends Tree[T] {
  override def map[K](f: T=>K): Tree[K] = Leaf(f(x))
...
case class Node[T] (x: T, children: List[Tree[T]]) extends Tree[T] {
  override def map[K](f: T => K): Tree[K] = Node(f(x), children.map(ch => ch.map(f)))
  override def toString: String = "Node(" + x.toString + "," + children.toString() + ")"
...
object Part2 {
  ...
  def breadthFirst[T](tr: Tree[T]): List[T] = {

    @tailrec
    def breadth(tr: List[Tree[T]], bs: List[T]) : List[T] = tr match {
      case Nil => bs
      case Leaf(x)::res => breadth(res, bs ++ List(x))
      case Node(x, children)::res => breadth(res ++ children, bs ++ List(x))
      case _ => List()
    }
    breadth(List(tr), List())
  }
  // Roughly 9 lines
  ...
  def reduce[T](f:(T, T) => T, y: T, L: List[T]): T = L match {
    case List() => y
    case x::xs => f(x, reduce(f, y, xs))
  }
  ...
  def sumTreeA(tr: Tree[A]): A = tr match {
    case Leaf(x) => x
    case Node(x, children) => reduce[A]((x, xs) => x.+(xs), x, children.map(ch => sumTreeA(ch)))
  }


  def getMinValue(other: Tree[T]):T = {
    other match {
      case Leaf(x) => x
      case Node(x, l, r) =>
